...
@torch.no_grad()
def sample_sequence(
    model, tokenizer, prompt: str = "", max_new_tokens: int = 100, top_k=None, temperature=1.0
):
    try:
        encoded_input = tokenizer(prompt, return_tensors="pt").to(device)
        inputs_ids = encoded_input["input_ids"]
        generated_tokens = model.generate(inputs_ids, max_new_tokens, top_k, temperature)
...
def main():
    ...
    sequence = sample_sequence(model, tokenizer, prompt, max_new_tokens, top_k, temperature)
    print(f"{sequence}")
